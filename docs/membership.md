---
layout: default
title: Membership
description: Join SLRCH — membership tiers and secure payment via PayPal.
eyebrow: Join SLRCH
lede: Membership supports the society's mission and is required for early career awards and reduced meeting registration. Payments are processed securely through PayPal.
extra_head: |
  <script src="https://www.paypal.com/sdk/js?client-id=BAA2hIzhLocCYLwUM-8VBSLeKxOXSv8TNPD3wXCtXlq4cwaXaiLHOK-1wCBeEjJ8I5fpVASW61lifOft7I&components=hosted-buttons&enable-funding=venmo&currency=USD"></script>
---

## Membership options

<div class="grid-2">

  <div class="tier">
    <h3>Regular</h3>
    <p class="price">$175 / year</p>
    <p class="description" style="font-size:0.95rem;color:var(--ink-soft);">For researchers and clinicians in the field.</p>
    <div id="paypal-container-5ANQPRFTC6Q3E"></div>
    <script>
      paypal.HostedButtons({
        hostedButtonId: "5ANQPRFTC6Q3E",
      }).render("#paypal-container-5ANQPRFTC6Q3E");
    </script>
  </div>

  <div class="tier">
    <h3>Student / Early Career</h3>
    <p class="price">$100 / year</p>
    <p class="description" style="font-size:0.95rem;color:var(--ink-soft);">For students, trainees, and postdocs within 5 years of their degree.</p>
    <div id="paypal-container-XX6D43W7NW64L"></div>
    <script>
      paypal.HostedButtons({
        hostedButtonId: "XX6D43W7NW64L",
      }).render("#paypal-container-XX6D43W7NW64L");
    </script>
  </div>

  <div class="tier">
    <h3>Retired</h3>
    <p class="price">$125 / year</p>
    <p class="description" style="font-size:0.95rem;color:var(--ink-soft);">For retired members of the society.</p>
    <div id="paypal-container-TC7BSHFLGSBYY"></div>
    <script>
      paypal.HostedButtons({
        hostedButtonId: "TC7BSHFLGSBYY",
      }).render("#paypal-container-TC7BSHFLGSBYY");
    </script>
  </div>

  <div class="tier">
    <h3>Corporate</h3>
    <p class="price">By arrangement</p>
    <p class="description" style="font-size:0.95rem;color:var(--ink-soft);">For companies and organizations. Please contact us directly to arrange corporate membership.</p>
    <p><a class="btn btn-secondary" href="mailto:contact.slrch@gmail.com">Contact us</a></p>
  </div>

  <div class="tier">
    <h3>Other</h3>
    <p class="price">By arrangement</p>
    <p class="description" style="font-size:0.95rem;color:var(--ink-soft);">Honorary members or other special circumstances not covered above — please get in touch.</p>
    <p><a class="btn btn-secondary" href="mailto:contact.slrch@gmail.com">Contact us</a></p>
  </div>

</div>

## How payment works

Clicking "Pay with PayPal" opens a secure PayPal checkout window. You can pay by PayPal balance, bank account, or credit/debit card — a PayPal account is not required to pay by card.

Questions about membership or payment? Contact [contact.slrch@gmail.com](mailto:contact.slrch@gmail.com).
